; Copy of org.zalando.stups.friboo.system.http
; As it the used version does not allow for custom middlewares.

(ns org.zalando.stups.pierone.http.http
    ...
    (condp #(.startsWith %2 %1) authorization
      ...
      "Basic " (let [basic-auth (parse-basic-auth authorization)]
                 (if (= (:username basic-auth) "oauth2")
                   ...
                   ; set parsed auth header if username is not oauth2
                   ; must be string because friboo's oauth2 protector does not check if arg is a string
                   (str (:username basic-auth) ":" (:password basic-auth))))
      authorization)))
